...
| [Seeed XIAO ePaper EE03 (10.3" mono)](https://www.seeedstudio.com/XIAO-ePaper-DIY-Kit-EE03-for-10-3-Monochrome-ePaper-Display.html?sensecap_affiliate=YcT0R1h&referring_service=link) | 1872×1404 | `gray_16` | `esp32_client` <br> `esp32_gray_bin` | `seeed_ee03` |
...
| [Seeed XIAO ePaper EE05 (2.13" BWRY)](https://www.seeedstudio.com/2-13-Quadruple-Color-ePaper-Display-with-122x250-Pixels-p-5779.html) | 250×122 | `bwry_4` | `esp32_client` (inherit) | `seeed_ee05_213_bwry` |
...
Runs the [Tesserae-native firmware](https://github.com/dmellok/tesserae-device-firmware). The PaperS3's glass has no controller chip: the ESP32-S3 drives the panel's parallel bus directly and the 16 greys come from a waveform table in the firmware, so greyscale tuning is a build-time edit rather than a vendor LUT. That also means no partial refresh yet, and so no tap echo or on-device touch widgets on this SKU. The original M5Paper shares the same 4.7" glass but reaches it through an IT8951 controller on the classic ESP32-D0WDQ6-V3, so it reuses the reTerminal E1003's greyscale driver unchanged; both M5Paper SKUs consume the identical 259200-byte frame.

| SKU | Panel | Gamut | Protocol / Renderer | Kind id |
|---|---|---|---|---|
| [M5Stack M5Paper](https://docs.m5stack.com/en/core/m5paper) | 540×960 portrait | `gray_16` | `esp32_client` <br> `esp32_gray_bin` | `m5stack_m5paper` |
| [M5Stack PaperMono](https://docs.m5stack.com/en/core/PaperMono) | 480×800 portrait | `gray_4` | `esp32_bw_client` <br> `esp32_gray2_bin` | `m5stack_papermono` |
...
|---|---|---|---|
| `circuitpython_bmp` | Raspberry Pi 3 + Waveshare 7" 1024x600 HDMI touch display, and Raspberry Pi 5 + Pi Touch Display 2 (10.1"), both running the community CircuitPython client under PyGame | :material-progress-helper: Partial | Confirmed on Pi-class hardware via the displayio PyGame emulation, pairing through the v1 REST discover flow as circuitpython_generic and painting the uncompressed indexed BMP with no zlib step. Held short of Tested because the microcontroller path the renderer was written for (Pico W class boards decoding via adafruit_imageload) is still unverified. Client: https://github.com/bablokb/tesserae-device-circuitpython |
| `circuitpython_png` | - | :material-circle-outline: Not yet tested | - |
| `esp32_bin` | Waveshare 13.3" Spectra 6 (ESP32-S3-WROOM-2) + Waveshare 7.3" PhotoPainter (ESP32-S3) + Seeed reTerminal E1002 and E1004 (Spectra 6) + Seeed XIAO 13.3" ePaper EE02 (Spectra 6) + Waveshare 10.85" e-Paper HAT+ (G) BWRY on an ESP32-S3 Zero via tesserae-device-firmware + paperlesspaper OpenPaper 7 (ESP32-C6, 7.3" Spectra 6 picture frame) via tesserae-device-firmware | :material-check-circle: Tested | Primary daily driver, battery-powered, deep-sleep. The 13.3" client lives at tesserae-device-esp32-bin; the 7.3" PhotoPainter client at tesserae-device-photopainter-7.3-bin. Both pair with the same renderer; the panel preset (waveshare_e6_13_3 vs waveshare_photopainter_7_3) selects the firmware-native row stride. The reTerminal Spectra 6 SKUs consume the same 4-bpp packed frames through tesserae-device-firmware. |
