...
| `esp32_gray2_bin` | Xteink X4 (GDEQ0426T82 / SSD1677, 800x480) running CrossInk with Tesserae client support | :material-check-circle: Tested | 4-level grayscale confirmed end-to-end: a 96000-byte 2-bpp frame fetched and composited on the panel as a base frame plus LSB/MSB planes, with distinct grey levels. Packing is byte-identical to the firmware's own 2-bpp bitmap reader, so no decode step is involved. Not yet confirmed on the Seeed reTerminal E1001 in its 4-gray mode. |
