...
| `TESSERAE_SECRET_KEY` | derived | 64 hex chars (32 bytes) used to encrypt stored secrets (API keys, broker passwords) at rest. Unset, the key is derived from the persisted session secret. That survives restarts, but if the session secret is ever regenerated (a data folder restored without `settings.json`, a container recreated without its data volume) every stored secret reads back empty while the rest of the settings stay intact. Pin it; see [Pin the secret key](docker.md#pin-the-secret-key-before-you-store-any-credentials). |
