// news_hacker_news — top N HN stories.
//
// Four directions pickable per-cell via `variant`, mirroring the rest
// of the family (weather, HA, calendar, todo):
//
//   r1  Refined    Dark Bauhaus header + accent lede block (#01 huge
//                  numeral + bold title + meta pills) + tinted list.
//   g2  Geometric  De Stijl colour blocks — every story is a tile,
//                  rank in mono, title in Archivo Black; rotating
//                  red/blue/yellow/green fills.
//   s3  Swiss      Hairline header, table of rows: 01 · title ·
//                  score / comments / ago, all tabular numerals.
//   d4  Data       Big top-score stat block, then horizontal score
//                  bars per story (relative to the leader).
...
function emptyState(label) {
  return `
    <div style="flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:6px;color:var(--wx-ink-60);font-family:var(--wx-grotesk)">
      ${WX.icon("flame", { size: 28, color: "var(--wx-ink-60)" })}
      <span style="font-size:12px;letter-spacing:.06em;font-weight:600;text-transform:uppercase">No ${escapeHtml((label || "stories").toLowerCase())}</span>
    </div>
  `;
}
...
// ===========================================================
function renderR1(data, stories) {
  const label = FEED_LABELS[data.feed] || "Hacker News";
  const headerRight = `${stories.length} STORIES · ${escapeHtml(nowTime())}`;
  const lede = stories[0];
  const rest = stories.slice(1);

  const meta = (s) => `
    <span style="display:inline-flex;align-items:center;gap:4px">
      ${WX.icon("arrow-fat-up", { size: 12, color: "currentColor" })}
      <span class="wx-tnum">${escapeHtml(fmtScore(s.score))}</span>
    </span>
    <span style="display:inline-flex;align-items:center;gap:4px">
      ${WX.icon("chat-circle-text", { size: 12, color: "currentColor" })}
      <span class="wx-tnum">${escapeHtml(fmtScore(s.comments))}</span>
    </span>
    <span style="display:inline-flex;align-items:center;gap:4px;opacity:.85">
      ${WX.icon("user", { size: 12, color: "currentColor" })}
      <span>${escapeHtml(s.by || "—")}</span>
    </span>
    <span style="display:inline-flex;align-items:center;gap:4px;opacity:.85">
      ${WX.icon("clock", { size: 12, color: "currentColor" })}
      <span class="wx-tnum">${escapeHtml(ago(s.time))}</span>
    </span>
  `;
      ...
      return `
        <article style="display:grid;grid-template-columns:auto 1fr;gap:14px;padding:8px 16px;border-bottom:1px solid color-mix(in oklab, var(--wx-ink) 10%, transparent);align-items:start">
          ...
          <div style="display:grid;gap:3px;min-width:0">
            <div style="font-family:var(--wx-grotesk);font-size:13px;font-weight:700;line-height:1.25;display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden">${escapeHtml(s.title)}</div>
            <div style="display:flex;flex-wrap:wrap;gap:10px;font-family:var(--wx-mono);font-size:10px;font-weight:700;letter-spacing:.04em;color:var(--wx-ink-60)">${meta(s)}</div>
          </div>
        </article>
      `;
    })
  ...
  return `
    ...
    <div class="wx-art" style="font-family:${DEFAULT_FONT};display:flex;flex-direction:column;background:var(--wx-paper)">
      ...
      <div style="flex:1;background:var(--wx-paper);overflow:hidden">
        ${stories.length === 0 ? emptyState(label) : rows || ""}
      </div>
    </div>
  `;
}
...
// ===========================================================
function renderG2(data, stories) {
  const label = FEED_LABELS[data.feed] || "Hacker News";
  // Rotate through accent fills. Order chosen so adjacent tiles never
  // share a colour even when wrapping to a new row.
  const PALETTE = [
  ...
  const cols = stories.length <= 4 ? 2 : stories.length <= 9 ? 3 : 4;

  const tiles = stories
    .map((s, i) => {
      const p = PALETTE[i % PALETTE.length];
      return `
        <div style="background:${p.bg};color:${p.fg};padding:12px 14px;display:flex;flex-direction:column;justify-content:space-between;gap:10px;min-width:0">
          <div style="display:flex;justify-content:space-between;align-items:baseline">
            ...
            <span style="font-family:var(--wx-mono);font-size:10px;letter-spacing:.06em;font-weight:700">▲ ${escapeHtml(fmtScore(s.score))}</span>
          </div>
          <div style="font-family:var(--wx-black);font-size:13px;line-height:1.15;font-weight:900;display:-webkit-box;-webkit-line-clamp:4;-webkit-box-orient:vertical;overflow:hidden;letter-spacing:-.005em">${escapeHtml(s.title)}</div>
          <div style="display:flex;justify-content:space-between;font-family:var(--wx-mono);font-size:9.5px;font-weight:700;letter-spacing:.08em;opacity:.92">
            <span>${escapeHtml(s.by || "—")}</span>
            <span class="wx-tnum">${escapeHtml(ago(s.time))} · ${escapeHtml(fmtScore(s.comments))} ✦</span>
          </div>
        </div>
      `;
    })
  ...
  return `
    ...
    <div class="wx-art" style="font-family:var(--wx-geo);display:flex;flex-direction:column;background:var(--wx-ink);gap:3px">
      <div style="background:var(--wx-ink);color:var(--wx-paper);padding:8px 14px;display:flex;justify-content:space-between;align-items:baseline;font-family:var(--wx-mono);font-size:12px;letter-spacing:.06em;font-weight:700;border-bottom:3px solid var(--c-accent)">
        ...
        <span style="opacity:.85;font-weight:400">${stories.length} STORIES</span>
      </div>
      ${stories.length === 0
        ? `<div style="flex:1;background:var(--wx-paper);display:flex">${emptyState(label)}</div>`
...
// ===========================================================
function renderS3(data, stories) {
  const label = FEED_LABELS[data.feed] || "Hacker News";
  const rows = stories
    .map((s, i) => {
      return `
        <div style="display:grid;grid-template-columns:24px 1fr auto auto auto;align-items:baseline;gap:14px;padding:7px 0;${i < stories.length - 1 ? "border-bottom:1px solid color-mix(in oklab, var(--wx-ink) 16%, transparent);" : ""}">
          ...
          <span style="font-size:13.5px;font-weight:500;color:var(--wx-ink);min-width:0;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${escapeHtml(s.title)}</span>
          <span class="wx-tnum" style="font-family:var(--wx-mono);font-size:10.5px;font-weight:700;letter-spacing:.06em;color:var(--c-accent)">${escapeHtml(fmtScore(s.score))}</span>
          <span class="wx-tnum" style="font-family:var(--wx-mono);font-size:10.5px;font-weight:700;letter-spacing:.06em;color:var(--wx-ink-60)">${escapeHtml(fmtScore(s.comments))}c</span>
          <span class="wx-tnum" style="font-family:var(--wx-mono);font-size:10.5px;font-weight:700;letter-spacing:.06em;color:var(--wx-ink-60)">${escapeHtml(ago(s.time))}</span>
        </div>
      `;
    })
    .join("");
  ...
  return `
    ...
    <div class="wx-art" style="font-family:var(--wx-swiss);padding:18px 24px;display:flex;flex-direction:column">
      <div style="display:flex;justify-content:space-between;align-items:baseline">
        ...
        <span style="font-size:10.5px;letter-spacing:.16em;color:var(--wx-ink-60)">${stories.length} stories · ${escapeHtml(nowTime())}</span>
      </div>
...
// ===========================================================
function renderD4(data, stories) {
  const label = FEED_LABELS[data.feed] || "Hacker News";
  const maxScore = stories.reduce((m, s) => Math.max(m, Number(s.score) || 0), 0) || 1;
  const top = stories[0];

  const bars = stories
    .slice(0, 8)
    .map((s, i) => {
      const w = (((Number(s.score) || 0) / maxScore) * 100).toFixed(1);
      const isTop = i === 0;
      return `
        <div style="display:grid;grid-template-columns:auto 1fr auto;align-items:center;gap:10px;padding:4px 14px;${i < Math.min(stories.length, 8) - 1 ? "border-bottom:1px solid color-mix(in oklab, var(--wx-ink) 10%, transparent);" : ""}">
          ...
          <div style="display:grid;gap:3px;min-width:0">
            <span style="font-family:var(--wx-grotesk);font-size:11.5px;font-weight:600;color:var(--wx-ink);min-width:0;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${escapeHtml(s.title)}</span>
            <div style="height:5px;background:var(--wx-paper-3)">
              <div style="width:${w}%;height:100%;background:${isTop ? "var(--c-accent)" : "var(--wx-blue)"}"></div>
            </div>
          </div>
          <span class="wx-tnum" style="font-family:var(--wx-black);font-size:13px;color:${isTop ? "var(--c-accent)" : "var(--wx-ink)"};min-width:42px;text-align:right">${escapeHtml(fmtScore(s.score))}</span>
        </div>
      `;
    })
    .join("");
  ...
  return `
    ...
    <div class="wx-art" style="font-family:${DEFAULT_FONT};display:flex;flex-direction:column;background:var(--wx-paper)">
      <div style="background:var(--wx-ink);color:var(--wx-paper);padding:10px 16px;display:flex;justify-content:space-between;align-items:baseline;border-bottom:3px solid var(--c-accent)">
        ...
        <span style="font-family:var(--wx-mono);font-size:10px;letter-spacing:.1em;opacity:.85">${stories.length} STORIES</span>
      </div>
      ...
      <div style="flex:1;background:var(--wx-paper);border-top:2px solid var(--wx-ink);overflow:hidden;padding:6px 0">
        ${stories.length === 0 ? emptyState(label) : bars}
      </div>
    </div>
  `;
}
...
// ===========================================================
function renderLegacy(data, stories) {
  const label = FEED_LABELS[data.feed] || "Hacker News";
  return `
    ${styleBlock()}
    <div class="wx-art" style="font-family:${DEFAULT_FONT};display:flex;flex-direction:column;background:var(--wx-paper)">
      ${WX.darkHeader({ title: label, accent: "ink", right: `${stories.length} STORIES · ${escapeHtml(nowTime())}` })}
      <div style="flex:1;overflow:hidden;border-top:2px solid var(--wx-ink)">
        ${stories.length === 0 ? emptyState(label) : stories.map((s, i) => {
          ...
          return `
            <article style="display:grid;grid-template-columns:auto 1fr;gap:14px;padding:8px 16px;border-bottom:1px solid color-mix(in oklab, var(--wx-ink) 10%, transparent);align-items:start">
              ...
              <div style="display:grid;gap:3px;min-width:0">
                <div style="font-family:var(--wx-grotesk);font-size:13px;font-weight:700;line-height:1.25;display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden">${escapeHtml(s.title)}</div>
                <div style="display:flex;flex-wrap:wrap;gap:10px;font-family:var(--wx-mono);font-size:10px;font-weight:700;letter-spacing:.04em;color:var(--wx-ink-60)">
                  <span>${escapeHtml(fmtScore(s.score))} ▲</span>
                  <span>${escapeHtml(fmtScore(s.comments))} ✦</span>
                  <span>${escapeHtml(s.by || "—")}</span>
                  <span>${escapeHtml(ago(s.time))}</span>
                </div>
              </div>
            </article>
          `;
...
export default async function render(shadow, ctx) {
  ...
  shadow.innerHTML = renderer(data, stories, size);
}
