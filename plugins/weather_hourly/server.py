...
def fetch(
    ...
    if cached is not None:
        ...
        # ``(lat, lon, units, hours)`` shows up on the next preview
        # instead of waiting for the cache TTL. Missing the ``place``
        # overlay was the v0.64.16 fix's gap; the new variants paint
        # ``place`` while the legacy ones paint ``label``.
