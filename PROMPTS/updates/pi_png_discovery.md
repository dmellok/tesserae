# Update: tesserae-pi-png-client → discovery heartbeat fields
...
Apply this **after** the [pi_png_multihead.md](pi_png_multihead.md)
...
| Key | Type | Source |
|---|---|---|
| `kind` | string — always `"pi_png_client"` | constant |
| `panel_w` | int (px) | resolved panel width |
| `panel_h` | int (px) | resolved panel height |
| `fw_version` | string | `importlib.metadata.version("tesserae-pi-png-client")` |
...
Wire `kind="pi_png_client"` as a constant. Pass `panel_w`, `panel_h`
...
2. **End-to-end** — in Tesserae's Settings → Devices, before
   ...
   appear with the right kind chip (`pi_png_client`) and the panel
