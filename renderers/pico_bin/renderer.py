...
Composition PNG packed into the panel-native landscape 4-bpp buffer for
the battery-powered Pico Plus 2 firmware
(``tesserae-device-pico-bin``) that drives a Pimoroni Inky-style
Spectra 6 panel over SPI. Byte-identical packing to ``pi_bin``;
content-addressed disk storage means both renderers share a single
file on disk when both targets are active.

What separates this from ``pi_bin``: published **retained**.
``pi_bin`` is non-retained because the Pi client is always awake on
MQTT, but the Pico client is a deep-sleep device that wakes once per
``sleep_interval_s``, reads the retained frame URL off
``tesserae/<device>/frame/bin``, paints, and sleeps. Without retain,
a freshly-woken client would block on a topic that's already been
delivered to the awake clients and miss the current frame.

What separates this from ``esp32_bin``: landscape-native packing.
``esp32_bin`` packs portrait-native (1200x1600 for the 13.3" Spectra 6),
matching the ESP32 firmware's on-device assumptions. The Pico firmware
uses the Pimoroni Inky library (or its C-port equivalent) which expects
the landscape-native buffer (1600x1200); the 90 deg rotate to native
panel orientation, plus the column-600 split between the two panel
controllers, happens on-device, so the renderer's job is just the
landscape buffer.
"""
